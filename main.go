...
func init()  {
	...
	//orm.RegisterDataBase("default", "mysql", "root:@/go?charset=utf8", 30)
	//// register model
...
func main() {
	...
	var UrlManager = func(ctx *context.Context) {
		...
		//	}
		//}
		ctx.Input.SetParam("key","SetParam我是过滤器")

	}

	beego.InsertFilter("/back/*",beego.BeforeRouter,UrlManager)
	beego.Run()
...
/*
    ...
    解析配置文件
    beego 会自动解析在 conf 目录下面的配置文件 app.conf，通过修改配置文件相关的属性，我们可以定义：开启的端口，
    是否开启 session，应用名称等信息。执行用户的 hookfunc
